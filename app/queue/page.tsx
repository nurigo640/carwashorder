...
export default function QueuePage() {
  ...
  return (
    <div style={{fontFamily:"Inter,sans-serif",background:'#f9f9ff',color:'#141b2b',minHeight:'100vh',display:'flex',flexDirection:'column',maxWidth:390,margin:'0 auto'}}>
      ...
      <main style={{flex:1,padding:'24px 16px',paddingBottom:200,display:'flex',flexDirection:'column',gap:12}}>
        {active.length === 0 ? (
            ...
            {active.map((entry) => {
              ...
              return (
                <div key={entry.id} style={{background:'#fff',borderRadius:12,padding:16,display:'flex',alignItems:'center',gap:16,border:'1px solid #c3c6d7',borderLeft:isWashing?'4px solid #2563eb':'1px solid #c3c6d7',boxShadow:isWashing?'0 4px 12px rgba(0,0,0,0.08)':'0 1px 3px rgba(0,0,0,0.06)'}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:16,fontWeight:700,letterSpacing:2,color:'#141b2b',textTransform:'uppercase'}}>{(entry as any).plate_number || entry.plate_masked}</div>
      ...
      <div style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:390,zIndex:50}}>
        <div style={{background:'linear-gradient(to top,#f9f9ff 60%,transparent)',padding:'32px 16px 12px'}}>
          ...
          <button onClick={() => router.push('/queue/join')} style={{width:'100%',marginTop:8,height:44,background:'transparent',color:'#434655',fontWeight:500,fontSize:14,borderRadius:12,border:'1px solid #c3c6d7',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
            Уже в очереди? Найти мою запись
          </button>
        </div>
